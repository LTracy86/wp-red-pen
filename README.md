...
- **REST namespace `wprp/v1`:** `GET/POST /notes`, `POST /notes/{id}` (edit), `POST /notes/{id}/status`, `POST /notes/{id}/replies`. Edit, status and delete require the `edit_posts` capability and the `wp_rest` nonce. Read and create additionally accept a valid reviewer-link token, sent as an `X-WPRP-Review-Token` header. The front-end panel is the main REST client; a connected Red Pen Hub can also pull notes over the same namespace.
...
- **Targeting context:** each note stores a `_wprp_ctx_key` (e.g. `post:12`, `pt_archive:composer`, `term:genre:5`, `tpl:single-track`, `search`, `404`), `_wprp_ctx_label`, and `_wprp_level` (page/template/global). The front end fetches by the current view's page + template keys. A one-time `init` migration backfills legacy notes as `post:` page notes (schema tracked in the `wprp_db_version` option). Visibility is the global `wprp_show_on` option.
- **Capability:** the dev surface is gated on `edit_posts` (editors + admins); permanent deletion additionally needs `delete_others_posts`. Ordinary logged-out visitors receive nothing.
- **Reviewer visibility:** a reviewer-link holder is anonymous, so every read they make runs through one gate, `wprp_reviewer_can_see_note()`. A note reaches them only if it is top-level, non-agent, on a public published page or template, and either carries the explicit `_wprp_client_visible` flag while open or was filed through that same link token (`_wprp_review_token`). The flag fails closed: no flag means internal. Screenshots are never served to a reviewer at all.
